...
export default async function handler(req) {
  ...
  const W          = 900;
  const ART_H      = 230; // Perfect visual height allocation to stop stretching
  const CTRL_H     = 76;  // Extended controls workspace zone
  const H          = ART_H + CTRL_H; // Total locked height calculation
  
  const PAD_X      = 28;
  const ROW2_TOP   = ART_H;
  const BRAND_X    = W - PAD_X;
  const BRAND_Y    = ROW2_TOP + 48;
...
<style>
  ...
  /* CRITICAL UI RENDERING FIX: Disables bilinear blurring on pixel graphics */
  .pixel-canvas {
...
<g clip-path="url(#fc)">
  ...
  <circle cx="22" cy="${ROW2_TOP + 18}" r="4.5" fill="#ff5f57"/>
  <circle cx="38" cy="${ROW2_TOP + 18}" r="4.5" fill="#febc2e"/>
  <circle cx="54" cy="${ROW2_TOP + 18}" r="4.5" fill="#28c840"/>
  
  <text x="74" y="${ROW2_TOP + 21}" font-family="'Courier New', Consolas, monospace" font-size="11" font-weight="700" fill="${c.dim}">connect · collaborate · build</text>
  <text x="292" y="${ROW2_TOP + 21}" font-family="'Courier New', Consolas, monospace" font-size="11" font-weight="700" fill="${c.accent}" class="terminal-cursor">█</text>

  <line x1="${PAD_X}" y1="${ROW2_TOP + 30}" x2="${W - PAD_X}" y2="${ROW2_TOP + 30}" stroke="${c.border}" stroke-width="0.5"/>
